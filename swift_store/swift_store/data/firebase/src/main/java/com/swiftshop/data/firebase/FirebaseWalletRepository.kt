...
class FirebaseWalletRepository @Inject constructor(
    ...
    override suspend fun initiateDeposit(
    ...
    ): Result<DepositInitiation> = runCatching {
        val data = mapOf(
        ...
        DepositInitiation(
            transactionId = resData["transactionId"] as? String ?: "",
            paymentUrl = resData["paymentUrl"] as? String,
            sessionId = resData["sessionId"] as? String
        )
    }

    override suspend fun verifyDeposit(sessionId: String): Result<Unit> = runCatching {
        functions.getHttpsCallable("verifyDeposit").call(mapOf("sessionId" to sessionId)).await()
        Unit
    }
