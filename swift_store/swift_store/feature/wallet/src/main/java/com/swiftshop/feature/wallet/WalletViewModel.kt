...
class WalletViewModel @Inject constructor(
    ...
    private val _depositInitiated = MutableSharedFlow<DepositInitiation>()
    val depositInitiated = _depositInitiated.asSharedFlow()
    ...
    fun verifyDeposit(sessionId: String) {
        viewModelScope.launch {
            _actionState.value = ActionState.Loading
            walletRepository.verifyDeposit(sessionId).fold(
                onSuccess = { _actionState.value = ActionState.Success("Deposit verified") },
                onFailure = { _actionState.value = ActionState.Error(it.message ?: "Verification failed") }
            )
        }
    }
